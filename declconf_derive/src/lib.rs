...
pub fn conf_derivation(input: TokenStream) -> TokenStream {
    ...
    match input.data {
        Data::Struct(ref data) => match data.fields {
            ...
            Fields::Named(ref fields) => gen_from_fields(struct_name, fields.clone()),
            Fields::Unnamed(_) => {
...
fn gen_from_fields(struct_name: Ident, fields: FieldsNamed) -> TokenStream {
                    ...
                    match (env_var_attrs.first(), cli_arg_attrs.first()) {
                        ...
                        (None, Some(cli_arg)) => match parse_attr_arg_name(cli_arg) {
                            Some(field_name_str) => quote! {
                                let #field_name = ::declconf::from_cli_arg(#field_name_str)
                            },
                            None => gen_compile_error!(
                                "Invalid cli_arg(NAME) meta attribute on struct field"
                            ),
                        },
                        (Some(env_var), None) => match parse_attr_arg_name(env_var) {
                            ...
                            None => gen_compile_error!(
                                "Invalid env_var(NAME) meta attribute on struct field"
                            ),
                        },
                    }
                }
